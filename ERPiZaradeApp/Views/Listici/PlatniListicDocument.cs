...
/// <summary>
/// Izgled platnog listića. Izdvojen iz code-behinda ekrana da bi isti dokument mogao da
/// se i štampa i šalje e-mailom — dva puta kroz različit kod značila bi da radnik dobije
/// drugačiji listić nego što knjigovođa vidi na ekranu.
/// </summary>
public static class PlatniListicDocument
{
    /// <summary>Jedan listić u fajl.</summary>
    public static void Sacuvaj(ObracunPlate o, string putanja)
        => Document.Create(c => c.Page(page => PodesiStranicu(page, o))).GeneratePdf(putanja);

    /// <summary>Više listića u jedan dokument, svaki na svojoj strani.</summary>
    public static void Sacuvaj(IEnumerable<ObracunPlate> obracuni, string putanja)
        => Document.Create(c =>
        {
            foreach (var o in obracuni) c.Page(page => PodesiStranicu(page, o));
        }).GeneratePdf(putanja);

    /// <summary>
    /// Jedan listić u memoriju — slanje e-mailom ne sme da ostavlja PDF sa JMBG-om i
    /// zaradom na disku duže nego što traje samo slanje.
    /// </summary>
    public static byte[] Generisi(ObracunPlate o)
        => Document.Create(c => c.Page(page => PodesiStranicu(page, o))).GeneratePdf();

    // ── POMOĆNA METODA: Konfiguracija izgleda jedne stranice platnog listića ──
    private static void PodesiStranicu(PageDescriptor page, ObracunPlate o)
    {

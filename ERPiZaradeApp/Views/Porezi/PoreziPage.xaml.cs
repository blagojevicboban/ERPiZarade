...
namespace ERPiZaradeApp.Views.Porezi;

public partial class PoreziPage : Page
{
    private PlataDbContext _db;
    private ERPiZaradeData.Models.Porezi? _currentParams;
    ...
    private void UcitajParametre(int godina, int mesec)
    {
        try
        {
            ...
            if (_currentParams == null)
            {
                ...
                if (fallback != null)
                {
                    _currentParams = new ERPiZaradeData.Models.Porezi
                    {
                        Godina = godina,
                        Mesec = mesec,
                        RedniBroj = 1,
        ...
        try
        {
            ...
            if (radniSatiList.Count > 0)
            {
                ...
                if (rez == MessageBoxResult.Yes)
                {
                    try
                    {
                        var obracunService = new ERPiZaradeApp.Services.ObracunService(_db);
